...
| Item | icon + name | Icon match vs bundled `public/images/items/*.png` sprites, cross-validated by the item **name text** shown below the ability; the icon's top-N shortlist is the text's candidate set |
...
2 images (any order)
  ...
  ├─ moves image: itemIcon match + item-name textMatch (icon top-N as candidates),
  │     textMatch for ability + 4 move rows
...
- **Localized item names**: items exist only as an English list in code plus icon PNGs — there is no `items` table. Add one (`identifier`, `name_en`, `name_ja`, `name_zh`, `name_zh_hans`, icon filename) from official localized data, in both DB copies, so item-name text matching works in all four languages.
...
2. **Item icons that look alike** at panel resolution. Mitigation: the item-name text below the ability cross-validates the icon match in v1 (icon top-N is the text's candidate set); remaining collisions land in the correction UI.
