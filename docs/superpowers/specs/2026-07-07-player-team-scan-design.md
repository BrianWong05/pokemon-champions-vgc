...
|---|---|---|
| Species | — (image only) | Existing ONNX classifier / descriptor on header sprite. **Never text** — panels show nicknames, so name text is untrustworthy by design |
| Ability | ≤3 names | `pokemon_abilities` for the classified species |
| Move (per row) | ~80–120 names | `pokemon_moves` learnset for the classified species. Move names are unique, so no type-icon filtering is needed |
| Item | full item list | Item **name text** (shown below the ability) matched against the app's legal item vocabulary. Item names are unique; the icon is not used |
...
2 images (any order)
  ...
  ├─ species: classifier/descriptor on header sprite (reuse; never from name text)
  ├─ moves image: textMatch for ability, item name, and 4 move rows
...
Text matching runs against all four vocabularies for each text region; the language whose candidates score best **in aggregate across the whole scan** (~24 move rows + 6 abilities + 6 item names is a strong signal) wins, and per-field results from that language are used. One language per scan; both images share it.
...
- **Localized item names**: items exist only as an English list in code plus icon PNGs — there is no `items` table. Add one (`identifier`, `name_en`, `name_ja`, `name_zh`, `name_zh_hans`) from official localized data, in both DB copies, so item-name text matching works in all four languages.
...
1. **Text-shape matching across fonts** (game font vs canvas system font) is unproven. Mitigations: bounded candidate sets, normalization, golden tests from day one. Contained fallback if a language underperforms: a per-language glyph atlas extracted from screenshots (the HP-reader path, generalized).
   - **Measured outcome + approved deviation (2026-07-07):** against the full vocabulary, EN golden accuracy plateaued at 35/36 top-1 (best font: Avenir Next 700 after a 15-font sweep; 7 feature variants tried) — mega-stone names are a confusable family and "Swampertite" loses to "Drampanite" by ~0.02. Decision: ship 35/36; the golden gate allows a named exception list where top-5 containment (rank slipped 2→5 when the missing mega stones were added to the vocabulary) + low-confidence flagging is required instead of top-1 (the correction UI pre-highlights these). The glyph atlas remains the follow-up if JA/zh golden accuracy disappoints. Note: candidate rendering uses the platform's font stack, so accuracy is platform-dependent (Android/WebView unverified) — the glyph atlas is also the platform-stable answer if that becomes a problem. Species policy: classifier candidates always primary on the player path; low-confidence appends descriptor candidates as correction options instead of replacing.
2. **Larger text vocabularies** (learnset-wide moves ~80–120, full item list ~hundreds) are harder than ≤3-candidate ability matching. Names are unique and visually distinct, so top-1 should hold; if goldens show collisions, the contained fallbacks are move-row type-icon filtering and item-icon descriptor matching (icon assets are already bundled) — both deliberately excluded from v1.
...
5. **v1 shipped state (2026-07-07):** EN fully green (with the approved Swampertite exception); zh-Hant/ja: stats/SP/nature 100% across goldens, residual = 2 zh species classifier misses + 3 zh/ja move-name shape collisions, all recoverable in the correction UI, tracked as `KNOWN_ISSUES` in `scripts/player-scan-accuracy.test.ts` (the e2e gate). Follow-ups: classifier hardening on panel crops, per-language glyph atlas. gameRect inference is not wired into the player pipeline — photos of a screen rely on manual crop-and-retry (spec Risk #3's mitigation is the crop flow, not gameRect).
6. **Glyph atlas shipped (2026-07-07, follow-up to #1/#5):** `scripts/build-text-glyph-atlas.ts` extracts per-character game-font templates from the labeled zh-Hant/ja golden moves screens (190 glyphs); `textMatch.ts` decodes each text crop into pitch-grouped cells and ranks candidates by per-glyph distance, with canvas shape matching as the sub-gate fallback (`ATLAS_ACCEPT`, calibrated on the builder's validation report: 71/71 labeled crops top-1, zero wrong tops). This closed all three zh/ja move-name `KNOWN_ISSUES` and the ja Delphoxite exception — though two of the four turned out to be data bugs the atlas *exposed* rather than shape losses: the ja-rental-r676 golden had slot-1 moves 3/4 swapped vs the screen, and `items.name_ja` for Delphoxite carried a mis-synthesized long-vowel mark (game renders マフォクシナイト). The atlas is also the platform-font-independent path flagged in #1 for covered strings (zh/ja); EN deliberately stays canvas-only — proportional Latin can't be column-segmented reliably and its gate is green. Known limitation: candidates containing a glyph absent from the atlas fall back to canvas matching; coverage grows with each labeled golden.
