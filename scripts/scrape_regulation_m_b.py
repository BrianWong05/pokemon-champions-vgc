...
def scrape_regulation_m_a():
    ...
    # M-B is additive over M-A: seed M-B with all of M-A's legal Pokemon, then add the
    ...
    if ma_row:
        ...
        print(f"Seeded M-B with {cursor.rowcount} Pokemon from M-A.")

    # No blacklist for M-B: the Megas excluded from M-A (Absol/Garchomp/Lucario Mega Z,
    # Mega Raichu X/Y) are exactly the M-B-exclusive content we want included here.
    blacklist_ids = set()
