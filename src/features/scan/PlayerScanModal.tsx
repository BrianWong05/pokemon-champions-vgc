...
const PlayerScanModal: React.FC<PlayerScanModalProps> = ({ isOpen, onClose, pokemonList, moveList, onSave, deps }) => {
  ...
  // vocab.items has duplicate name_en rows (distinct version-group entries with the same
  // display name) — dedupe for the datalist, which only cares about unique visible values.
  ...
  // Picking a different species invalidates that slot's ability/item/moves (they belonged
  // to the old species). setSlotSpecies rescans and produces fresh merged data for the slot;
  // drop the local edit so the seeding effect re-derives it from that fresh data.
  ...
  const cropRetry = (kind: PlayerScreenKind) => (b: Blob) => {
  ...
  // Notice names the screen still MISSING, when exactly one of the two is done.
    ...
    return (
      <div key={kind} className="flex-1 p-3 rounded border border-line-2 space-y-2">
        ...
        {croppingKind === kind && state.blob ? (
          <CropStep blob={state.blob} onCropped={cropRetry(kind)} onCancel={() => setCroppingKind(null)} />
        ) : (
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Scan my team" maxWidth="max-w-4xl">
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <button className="px-4 py-2 rounded border border-line-2 text-ink-2 hover:bg-raise" onClick={handleClose}>
            Cancel
          </button>
          <button
